...
void Texture2D::pushToGPU(bool deleteAfterPush) {
	...
	glTextureStorage2D(textureID, mipmapLevels, imageData.format.sizedFormat, imageData.width, imageData.height); //allocate space for all

	if (imageData.data != 0)
		glTextureSubImage2D(textureID, 0, 0, 0, imageData.width, imageData.height, imageData.format.baseFormat, imageData.format.type, imageData.data); //level 0 mipmap
