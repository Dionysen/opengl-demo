...
void FBO::pushToGPU(bool deleteAfterPush)
{
    ...
    for (int i = 0; i < colorAtachmentCount; ++i) { drawBuffers[i] = GL_COLOR_ATTACHMENT0 + i; }
...
{
    glNamedFramebufferTexture(name, GL_COLOR_ATTACHMENT0 + colorAttachmentNumber, texture.textureID, 0);
    colorAtachmentCount++;
}

void FBO::attachTextureToDepthBuffer(const Texture& texture)
{
    glNamedFramebufferTexture(name, GL_DEPTH_ATTACHMENT, texture.textureID, 0);
}

void FBO::attachTextureToStencilBuffer(const Texture& texture)
{
    glNamedFramebufferTexture(name, GL_STENCIL_ATTACHMENT, texture.textureID, 0);
}

void FBO::attachTextureToDepthAndStencilBuffer(const Texture& texture)
{
    glNamedFramebufferTexture(name, GL_DEPTH_STENCIL_ATTACHMENT, texture.textureID, 0);
}