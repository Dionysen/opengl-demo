...
{
    glTextureStorage3D(textureID, mipmapLevels, images[0].format.sizedFormat, images[0].width, images[0].height,
                       images.size());  // allocate space for all
    ...
    for (unsigned int i = 0; i < images.size(); ++i)
    {
        ...
        if (imageData.data != 0)
            glTextureSubImage3D(textureID, 0, 0, 0, i, imageData.width, imageData.height, 1,
