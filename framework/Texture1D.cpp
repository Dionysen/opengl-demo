...
void Texture1D::pushToGPU(bool deleteAfterPush)
{
    ...
    glTextureStorage1D(textureID, mipmapLevels, imageData.format.sizedFormat,
                       imageData.width);  // allocate space for all

    if (imageData.data != 0)
        glTextureSubImage1D(textureID, 0, 0, imageData.width, imageData.format.baseFormat, imageData.format.type,
