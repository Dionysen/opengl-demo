...
class Utils
{
  public:
    ...
    inline static float clamp(const float v, const float lo, const float hi) { return std::max(lo, std::min(hi, v)); }

    inline static float degToRad(const float deg) { return (PI / 180.0f * deg); }

    inline static bool floatEquals(const float value, const float comp, const float epsilon = EPSILON_FLOAT) { return abs(value - comp) <= epsilon; }
