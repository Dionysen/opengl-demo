...
Application::~Application()
{
    ...
    for (SceneObject* object : sceneObjects) { delete object; }
...
{
    for (SceneObject* object : sceneObjects) { object->initialize(); }
    initialized = true;
}
...
void Application::keyCallback(const int key, const int scanCode, const int action, const int mods)
{
    ...
    for (EventListener* el : eventListeners) { el->keyCallback(key, scanCode, action, mods); }
}
...
void Application::cursorPositionCallback(const double xpos, const double ypos)
{
    ...
    for (EventListener* el : eventListeners) { el->cursorPositionCallback(xpos, ypos); }
}
...
void Application::mouseButtonCallback(const int button, const int action, const int mods)
{
    ...
    for (EventListener* el : eventListeners) { el->mouseButtonCallback(button, action, mods); }
}
