...
{
    for (std::string path : paths) { images.push_back(loadFile(path, format)); }
    pushToGPU();
}
...
const ImageData Texture::loadFile(const std::string& path, const ImageFormat& format)
{
    ...
    // unsigned char* data = stbi_load(path.c_str(), &imageData.width,
    // &imageData.height, &nrComponents, 0);
    //// check format
    // if (nrComponents != imageData.format.numberOfChannels)
    //{
    //     std::cout << "Failed to load image from file: " << path << "[Format
    //     error!\n]" << std::endl;
    // }
    //// check data
    // if (data)
    //{
    //     imageData.sizeInBytes = imageData.width * imageData.height *
    //     imageData.format.numberOfChannels; imageData.data = new unsigned
    //     char[imageData.sizeInBytes]; memcpy(imageData.data, data,
