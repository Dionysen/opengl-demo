...
#define K_VEC(n, m) \
    vec2(2 * M_PI * (n - N / 2) / L_x, 2 * M_PI * (m - M / 2) / L_z)

Wave::Wave(int N, int M, float L_x, float L_z, vec2 omega, float V, float A,
           float lambda)
    : N(N), M(M), omega_hat(normalize(omega)), V(V), L_x(L_x), L_z(L_z), A(A),
      lambda(lambda)
{
...
// Eq14
inline float Wave::func_omega(float k) const { return sqrt(g * k); }
...
inline float Wave::func_P_h(vec2 vec_k) const
{
    ...
    float result =
        A * exp(-1 / (k * L * k * L)) / pow(k, 4) * pow(dot_k_hat_omega_hat, 2);
...
inline complex<float> Wave::func_h_twiddle(int kn, int km, float t) const
{
    ...
    complex<float> term1 =
        value_h_twiddle_0[index] * exp(complex<float>(0.0f, func_omega(k) * t));
    complex<float> term2 = value_h_twiddle_0_conj[index] *
                           exp(complex<float>(0.0f, -func_omega(k) * t));
    return term1 + term2;
}
...
void Wave::buildField(float time)
{
    ...
    for (int n = 0; n < N; n++)
        for (int m = 0; m < M; m++)
        {
            ...
            slope_x_term[index] =
                complex<float>(0, kVec.x) * value_h_twiddle[index];
            slope_z_term[index] =
                complex<float>(0, kVec.y) * value_h_twiddle[index];
            D_x_term[index] =
                complex<float>(0, -kVecNormalized.x) * value_h_twiddle[index];
            D_z_term[index] =
                complex<float>(0, -kVecNormalized.y) * value_h_twiddle[index];
        }
    ...
    p_height = fftwf_plan_dft_2d(N, M, in_height, out_height, FFTW_BACKWARD,
                                 FFTW_ESTIMATE);
    p_slope_x = fftwf_plan_dft_2d(N, M, in_slope_x, out_slope_x, FFTW_BACKWARD,
                                  FFTW_ESTIMATE);
    p_slope_z = fftwf_plan_dft_2d(N, M, in_slope_z, out_slope_z, FFTW_BACKWARD,
                                  FFTW_ESTIMATE);
    p_D_x =
        fftwf_plan_dft_2d(N, M, in_D_x, out_D_x, FFTW_BACKWARD, FFTW_ESTIMATE);
    p_D_z =
        fftwf_plan_dft_2d(N, M, in_D_z, out_D_z, FFTW_BACKWARD, FFTW_ESTIMATE);
    ...
    for (int n = 0; n < N; n++)
        for (int m = 0; m < M; m++)
        {
            ...
            normalField[index] =
                normalize(vec3(sign * out_slope_x[index][0], -1,
                               sign * out_slope_z[index][0]));

            heightField[index] =
                vec3((n - N / 2) * L_x / N - sign * lambda * out_D_x[index][0],
