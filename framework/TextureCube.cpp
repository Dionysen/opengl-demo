...
{
    glTextureStorage2D(textureID, mipmapLevels, images[0].format.sizedFormat, images[0].width,
                       images[0].height);  // allocate space for all
    bind();
    ...
    for (int i = 0; i < 6; ++i)
    {
        ...
        if (imageData.data != 0)
            glTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, 0, 0, imageData.width, imageData.height, imageData.format.baseFormat,
                            imageData.format.type, imageData.data);  // level 0 mipmap
